...
$outdir ||= ".";
my $mapDir = dirname (dirname($koMap)) . "/map";
$mapDir = "/nfs/database/db/Pub/kegg/RNA/59.3/map" unless (-d "$mapDir");
sub usage {
	print STDERR << "USAGE";
description: draw KEGG Map, referenced from kegg_parser.pl
...
$map_dir .= "\../map";
#my $map_htmls = `ls $map_dir/*.html`; #error?
my $map_htmls = `ls $mapDir/*.html`; #modfied by liangq 20150122 error?
my @maps_arr = split(/\n/, $map_htmls);
