...
open (IN, '<', $in);
open (OUT, '>', $out);
my ($AA_ens,$AA_Change_ens,@AA_Change_ens,$AA_ref,$AA_Change_ref,@AA_Change_ref);
while (<IN>) {
	chomp;
	my $line=$_;
	my @line=split (/\t/,$line);
	if ((/^#/) || (/^chr/)) {
		for (my $i=0;$i < @line;$i++) {
			if ($line[$i] eq "AAChange.ensGene") {
				$AA_ens=$i;
			}
			if ($line[$i] eq "AAChange.refGene") {
				$AA_ref=$i;
			}
		}
		if ($t eq "germline") {
	...
	$AA_Change_ens=$line[$AA_ens];
	$AA_Change_ref=$line[$AA_ref];
	@AA_Change_ens=split (",", $AA_Change_ens);
	@AA_Change_ref=split (",", $AA_Change_ref);
	if (@AA_Change_ens >10) {
		@line[$AA_ens]=join(",",@AA_Change_ens[1..10]);
	}
	if (@AA_Change_ref >10) {
		@line[$AA_ref]=join(",",@AA_Change_ref[1..10]);
	}
