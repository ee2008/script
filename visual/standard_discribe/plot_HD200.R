...
library("gtable")
grid.newpage()
path_data <- "/lustre/project/og04/wangxian/visual_test/standard_discribe/HD200.txt"
dd_raw <- read.table(path_data, sep = "\t", header = T, stringsAsFactors = T)
dd <- stack(dd_raw)
colnames(dd) <- c("fre","gene")
dd_raw_nrow <- nrow(dd_raw)
dd$standard <- rep(c(0.105,0.10,0.03,0.01,0.245,0.15,0.06,0.125,0.175,0.09,0.02),rep(dd_raw_nrow,ncol(dd_raw)-1))
...
p1 <- ggplot(dd) + geom_point(aes(x = gene, y = fre), colour = "black", size = 2)  + theme_bw() + ylab("Alt_Fre") + theme(panel.grid.major = element_line(colour = NA), panel.background = element_rect(fill = "grey90", colour = "black")) + ggtitle("HD200") + geom_point(aes(x = gene, y = standard), colour = "red", size = 3, pch = 17) + theme(axis.text.x=element_text(size=10,angle=90,color="black"))
