...
本文件汇总了现有 CN500 测序仪肿瘤样本测序数据的结果, 并从以下角度和步骤进行总结
...
panel_type <- function(string) {
    panel_split <- strsplit(x = string, split = '_')[[1]]
    panel <- panel_split[length(panel_split)]
    if (grepl(pattern = 'P2', x = panel, fixed = T)) {
        return('P1P2')
    } else if (grepl(pattern = 'moregene', x = panel, fixed = T)) {
        return('BRCA')
    } else {
        return('P1')
    }
...
extract_date <- function(string) {
    ...
    date <- strsplit(strsplit(x = prj, split = '_')[[1]][1], '-')[[1]][3]
    return(date)
}
...
# ==== load data ====
#dd <- read.delim("/p299/project/og04/shenzhongji2/CA-PM/all_qc_stats/all_qc_stats.tsv", as.is = T, na.strings = 'NA', header = F)
...
latest10 <- tail(dd_date,n=10)
dd <- subset(dd, date %in% latest10)
# add eff
...
```{r eff_seq~seq_dep}
ggplot(dd) + geom_point(aes(seq_dep, panel_dep)) + geom_smooth(aes(seq_dep, panel_dep)) + facet_wrap(~sample_type, scales = 'free_x') + labs(title = 'seq eff')
...
```{r GC, warning=FALSE}
ggplot(dd) + geom_point(aes(GC, eff_seq, color = date, size = panel_type, shape = sample_type), alpha = 0.6) + stat_smooth(aes(GC, eff_seq)) + geom_vline(xintercept = panel12_GC, color = "red") + geom_text(aes(x = panel12_GC, label = paste0("panel GC: ", panel12_GC), y = 0)) + labs(title = 'eff_seq ~ GC') + geom_vline(xintercept = c(panel12_GC - 2, panel12_GC + 2), color = "grey")
ggplot(dd) + geom_point(aes(GC, eff_seq, color = sample_type), alpha = 0.6) + stat_smooth(aes(GC, eff_seq)) + geom_vline(xintercept = panel12_GC, color = "red") + geom_text(aes(x = panel12_GC, label = paste0("panel GC: ", panel12_GC), y = 0)) + labs(title = 'eff_seq ~ GC by sample') + facet_wrap(~sample_type, scales = "free_x") + geom_vline(xintercept = c(panel12_GC - 2, panel12_GC + 2), color = "grey")
ggplot(dd) + geom_point(aes(GC, eff_seq, color = sample_type), alpha = 0.6) + stat_smooth(aes(GC, eff_seq)) + geom_vline(xintercept = panel12_GC, color = "red") + geom_text(aes(x = panel12_GC, label = paste0("panel GC: ", panel12_GC), y = 0)) + labs(title = 'eff_seq ~ GC by sample by date') + facet_wrap(~date, ncol = 3, scales = "free") + geom_vline(xintercept = c(panel12_GC - 2, panel12_GC + 2), color = "grey")
```
...
> GC 含量影响数据利用率, 合理范围为预期 GC% +- 2% (如 NIPT), 对应范围为 `r panel12_GC - 2` 至 `r panel12_GC + 2`
...
```{r latest5, echo=FALSE, warning=FALSE}
dd_date <- levels(factor(dd$date))
latest5 <- tail(dd_date,n=5)
latest5_name <- paste(latest5[1:5],collapse="、")
dd_latest5 <- subset(dd, date %in% latest5)
```

最新五批 `r latest5` 共有 `r nrow(dd_latest5)` 个样本

```{r latest5_sample_type, echo=FALSE}
table(dd_latest5$sample_type)
```

## 所有质控指标一览图

```{r latest5_plot_p1, echo=FALSE, message=FALSE, warning=FALSE}
library(reshape2)
dd_latest5_long <- melt(dd_latest5[, c(1:ncol(dd_latest5))])
ggplot(dd_latest5_long) + geom_boxplot(aes(x = sample_type, y = value, color = panel_type), alpha = 0.3) + geom_violin(aes(sample_type, value, color = panel_type), alpha = 0.6) + geom_jitter(aes(sample_type,  value, color = panel_type), size = 0.2) + labs(title = paste0('boxplot of all QC for ', latest5_name)) + facet_wrap(~variable, scales = 'free_y', ncol = 3)
```

## 各指标各自展示图

```{r latest5_plot_p2, echo=FALSE, message=FALSE, warning=FALSE}
library(dplyr)
is_outlier <- function(x) {
  return(x < quantile(x, 0.25) - 1.5 * IQR(x) | x > quantile(x, 0.75) + 1.5 * IQR(x))
}

for (i in levels(dd_latest5_long$variable)) {
    ddd5 <- subset(dd_latest5_long, variable == i & !is.na(value))
    print(ggplot(ddd5) + geom_boxplot(aes(x = sample_type, y = value, color = panel_type), alpha = 0.3) + geom_violin(aes(sample_type, value, color = panel_type), alpha = 0.6) + geom_jitter(aes(sample_type,  value, color = panel_type), size = 1.0) + labs(title = paste0(latest5_name, ": ", i)))
