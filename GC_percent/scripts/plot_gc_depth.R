...
#data_path="/lustre/project/og04/pub/pipeline/plot_gc_depth/CFD/CA-PM.gc_depth_20170427_162209.txt"
...
## plot line
n_sample <- table(data_all$index)
n_sample <- n_sample-1
data_line <- data_all[complete.cases(data_all$gc),]
gc_index <- data_all[complete.cases(data_all$gc_stand),]
data_line$gc_stand <- rep(gc_index$gc_stand,n_sample) 
data_line <- data_line[order(data_line$gc_stand),]
output_path1 <- paste0(out_dir,"/depth_gc.png")
png(output_path1,width=2000,height=2500,units="px")
#p <- ggplot() + geom_line(data=data_line,aes(x=gc_stand,y=gc,color=as.factor(project)),alpha = 0.3,size=0.5) + geom_line(data=data_line,aes(x=gc_stand,y=100*log10(depth),color=as.factor(project)),alpha = 0.3,size=0.5,lty=2) + ylab("GC_percent(%) and Depth") + xlab("GC") + theme(axis.title.y=element_text(face="bold",size=15),axis.text.y=element_text(face="bold",size=10,color="black"),axis.text.x=element_text(face="bold",size=8,angle=90,color="black"),title=element_text(face="bold",size=20),legend.position="right",legend.direction="vertical",legend.title=element_text(face="bold",size=10)) + labs(fill=name_legend) + labs(color=name_legend)
p <- ggplot() + geom_point(data=data_line,aes(x=gc_stand,y=gc,color=as.factor(project)),alpha = 0.3,size=0.5) + geom_point(data=data_line,aes(x=gc_stand,y=100*log10(depth),color=as.factor(project)),alpha = 0.3,size=0.5,pch=17) + ylab("GC_percent(%) and Depth") + xlab("GC") + theme(axis.title.y=element_text(face="bold",size=15),axis.text.y=element_text(face="bold",size=10,color="black"),axis.text.x=element_text(face="bold",size=8,angle=90,color="black"),title=element_text(face="bold",size=20),legend.position="right",legend.direction="vertical",legend.title=element_text(face="bold",size=10)) + labs(fill=name_legend) + labs(color=name_legend)
print(p)
dev.off()
